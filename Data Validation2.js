...
function validateTasks(tasks, subDivisionAmount) {
  ...
  // Find the minimum and maximum dates by grouping the data of tasks
...
console.log(`Maximum date: ${maxDate}`);
  ...
  // Find the minimum and maximum dates by grouping the data of  sub division
function findMinMaxDates(data) {
  let minDate = null;
  let maxDate = null;
...
console.log(`Maximum date: ${maxDate}`);
  ...
  function validateDivisions(divisions) {
    for (let division of divisions) {
      ...
      const divStartDate = new Date(divisions.divStartDate);
      const divEndDate = new Date(divisions.divEndDate);
      const durationDays = (divEndDate - divStartDate) / (1000 * 60 * 60 * 24);
      
  // Find the minimum and maximum dates by grouping the data of division
function findMinMaxDates(data) {
  let minDate = null;
  let maxDate = null;
...
console.log(`Maximum date: ${maxDate}`);
      ...
      if (durationDays !== divisions.originalDurationDays) {
        console.log(`Invalid Divisions duration for ${divisions.divisionsNumber}. Original duration: ${divisions.originalDurationDays}, actual duration: ${durationDays}`);
        return false;
      }

      if (divStartDate >= divEndDate) {
        console.log(`Invalid division dates for ${divisions.divisionNumber}. End date should be after start date.`);
        return false;
      }
    return {
      startDate:"",
